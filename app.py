...
# --- 1. FUNZIONI MEMORIA FISSA (DATABASE LOCALE) ---
...
def carica_dati():
    if os.path.exists(DB_FILE):
        with open(DB_FILE, "r") as f:
            d = json.load(f)
            # Converte la stringa ISO in oggetto data
...
# -- BOX --
    ...
    if qualita == "Standard":
        for stile, classe, desc in [("LUNA 🌙", "box-luna", "Neutro"), ("SOLE ☀️", "box-sole", "Vivace"), ("NUVOLA ☁️", "box-nuvola", "Grigio nuvola")]:
            st.markdown(f'<div class="card {classe}"><h3>{stile}</h3><p>{desc}</p><div class="prezzo-rosa">19,90€</div></div>', unsafe_allow_html=True)
            if st.button(f"Scegli {stile}", key=f"std_{stile}"): st.success(f"{stile} aggiunta!")
    else:
        st.markdown('<div class="card box-premium"><h3>BOX PREMIUM 💎</h3><p>Il meglio per il tuo bimbo</p><div style="font-size:28px; font-weight:900;">29,90€</div></div>', unsafe_allow_html=True)
        if st.button("Scegli Box Premium", key="prem_u"): st.success("Premium aggiunta!")
...
# -- PROFILO (BLINDATO CON MEMORIA FISSA) --
    ...
    if not st.session_state.edit_mode:
        ...
        with st.form("edit_profilo"):
            ...
            taglia = st.selectbox("Taglia", ["50-56 cm (0-3m)", "62-68 cm (3-6m)", "74-80 cm (6-12m)", "86-92 cm (12-24m)"], index=0)
            
            if st.form_submit_button("🔍 Trova Locker più vicino"):
                st.session_state.temp_locker = "Locker Via Roma 10, Calolziocorte (LC)"
            
            lock = st.text_input("Indirizzo Locker", st.session_state.dati['locker'])
            
            if st.form_submit_button("SALVA E BLOCCA DATI"):
                st.session_state.dati = {"nome_genitore": nome, "email": mail, "telefono": tel, "nome_bambino": nome_b, "nascita": nascita, "taglia": taglia, "locker": lock}
