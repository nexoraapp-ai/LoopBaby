...
def carica_db():
    try:
        ...
        if res.status_code == 200:
            ...
            if not df.empty:
                # Pulizia automatica: toglie spazi dai nomi delle colonne
                df.columns = [c.strip() for column in [df.columns] for c in column]
            return df
        return pd.DataFrame()
...
if st.session_state.user is None:
            ...
            with st.form("login"):
                ...
                if st.form_submit_button("ENTRA"):
                    if not df.empty and 'email' in df.columns:
                        user_match = df[(df['email'] == e) & (df['password'].astype(str) == str(p))]
                        if not user_match.empty:
                            st.session_state.user = user_match.iloc[0].to_dict()
                            vai("Home")
                        else: st.error("Dati errati")
                    else: st.error("Database in aggiornamento o vuoto.")
        with t2:
            with st.form("reg"):
                er = st.text_input("Tua Email", key="r_e")
                pr = st.text_input("Password", key="r_p")
                if st.form_submit_button("CREA ACCOUNT"):
                    if not df.empty and 'email' in df.columns and er in df['email'].values:
                        st.error("Esiste già!")
                    else:
                        ...
                        st.success("Account creato! Fai l'accesso.")
else:
    # UTENTE LOGGATO
    ...
    if scad_str != 'nan' and scad_str != '':
        try:
            ...
            if 0 <= giorni <= 10:
                st.markdown(f'<div class="avviso-scadenza">⚠️ Mancano {giorni} giorni al termine! Cambiamo taglia?</div>', unsafe_allow_html=True)
        except: pass
