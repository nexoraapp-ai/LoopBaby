...
# --- 4. SESSIONE (Inizializzazione Sicura) ---
...
if not st.session_state.autenticato:
    ...
    if st.session_state.verificando_mail:
        ...
        cod_in = st.text_input("Inserisci il codice ricevuto via mail", key="verify_cod")
    ...
    with t2:
        n = st.text_input("Nome", key="reg_n")
        em = st.text_input("Email", key="reg_em")
        tl = st.text_input("Cellulare", key="reg_tl")
        if st.button("REGISTRATI"):
            if n and em and tl:
                st.session_state.codice_segreto = random.randint(100000, 999999)
                st.session_state.temp_email = em
                st.session_state.user_data["nome"] = n
                st.session_state.user_data["email"] = em
                st.session_state.user_data["cellulare"] = tl
...
if scelta == "🏠 Home":
    ...
    <div class="story-card">
        <h4>Chi Siamo? 👨‍👩‍👧</h4>
        <p>Siamo <b>genitori</b> che capiscono la necessità di cambiare i vestiti ogni mese. Abbiamo creato <b>LoopBaby</b> per eliminare lo stress, lo spazio sprecato e i costi eccessivi.</p>
    </div>
    <div class="card">
...
# --- 8. PAGINA PROFILO ---
    ...
    with st.form("edit_profilo"):
        ...
        new_tg = st.selectbox("Taglia attuale bimbo/a", ["0-1m", "1-3m", "3-6m", "6-9m", "12-24m"])
...
# --- 11. VETRINA ---
    ...
    st.warning("I capi acquistati qui rimarranno a te! Non sono soggetti al riciclo LoopBaby.")
    st.write("Spedizione GRATIS sopra i 50€ o associata a una box.")
    # Esempio prodotto
...
# --- 12. ADMIN ---
    ...
    else: st.error("Accesso solo Admin")
