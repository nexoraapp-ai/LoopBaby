...
# Gestione navigazione per il link "contattaci" nelle Info
...
logo_bg = get_base64("logo.png") # CARICAMENTO LOGO PNG

# --- 3. CSS TOTALE (BEIGE + NUOVO HEADER PANORAMICO) ---
st.markdown(f"""
    <style>
    ...
    * {{ font-family: 'Lexend', sans-serif !important; }}
    ...
    /* HEADER PANORAMICO CON IMMAGINE DI SFONDO */
    .header-custom {{
...
# -- DETTAGLIO PROMO --
    ...
    st.markdown("""<div class="card" style="text-align:left; font-size:14px; line-height:1.6;">
            <b>Preparare il pacco è semplicissimo:</b> mandaci almeno <b>10 capi</b> in buono stato, noi paghiamo il trasporto e ti regaliamo la tua <b>prima Box</b> da usare entro 3 mesi!<br><br>
            Ti basterà fornirci peso e dimensioni per ricevere l'etichetta di spedizione gratuita da applicare al pacco.
            </div>""", unsafe_allow_html=True)
    with st.form("promo_form"):
        p = st.text_input("Peso stimato (kg)")
        d = st.text_input("Dimensioni (Lunghezza x Larghezza x Altezza)")
...
# -- INFO --
    ...
    st.markdown("""<div class="card" style="text-align:left; font-size:13px; color:#475569; line-height:1.6;">
            La Box LoopBaby ha un costo di 19,90€ (Standard) o 29,90€ (Premium). Se rinnovi l'ordine, il ritiro della precedente è GRATUITO. Se invece desideri restituire senza nuovo ordine, il ritiro costa 7,90€.<br><br>
            <b>📍 La Regola del 10:</b> Rendi 10 capi per riceverne 10. Se un capo viene smarrito, vale lo scambio <b>'Jeans x Jeans'</b> o una penale di 5 euro a capo mancante.
        </div>""", unsafe_allow_html=True)
...
# -- BOX --
    ...
    if q == "Standard":
        for s, c, d in [("LUNA 🌙", "box-luna", "Neutro (Bianco, Panna, Grigio)"), ("SOLE ☀️", "box-sole", "Vivace (Colori e Fantasie)"), ("NUVOLA ☁️", "box-nuvola", "Grigio nuvola (Denim)")]:
        ...
        st.markdown('<div class="card box-premium"><h3>BOX PREMIUM 💎</h3><p>Capi nuovi o seminuovi delle migliori marche</p><div style="font-size:28px; font-weight:900;">29,90€</div></div>', unsafe_allow_html=True)
...
# -- VETRINA --
    ...
    st.markdown("""<div style="text-align:center; padding: 0 20px; color:#475569; font-size:14px; margin-bottom:20px; line-height:1.5;">
            ...
            Spedizione <b>GRATUITA</b> sopra i 50€ o se associata alla consegna della tua prossima Box.
        </div>""", unsafe_allow_html=True)
...
# -- PROFILO --
    ...
    if not st.session_state.edit_mode:
        st.markdown(f"""<div class="card" style="text-align:left; font-size:14px;">
            ...
            <b>📍 Locker scelto:</b> {st.session_state.dati['locker']}
        </div>""", unsafe_allow_html=True)
...
# -- CHI SIAMO --
    ...
    st.markdown("""
        <div style="padding: 0 20px; font-size:14px; color:#475569; line-height:1.6; text-align:center;">
            Abbiamo vissuto sulla nostra pelle quanto sia impegnativo far crescere un bambino. Per questo abbiamo creato LoopBaby: per semplificarti la vita e ridurre gli sprechi.
        </div>
