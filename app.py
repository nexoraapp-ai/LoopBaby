...
# --- 1. CONNESSIONE SUPABASE (PULITA) ---
# Forziamo le stringhe per evitare errori di tipo (TypeError)
...
# Inizializzazione diretta senza cache per debuggare l'errore
try:
...
def login(e, p):
    try:
        ...
        if d.data: st.session_state.dati = d.data[0] # Prendi il primo risultato
        vai("Home"); st.rerun()
...
def registro(e, p):
    try:
        ...
        st.success("📩 Mail inviata! Controlla la posta (anche Spam).")
