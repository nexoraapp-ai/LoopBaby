...
# Gestione link "contattaci" tramite query params
...
# --- 3. CSS TOTALE (Stile Originale + Fix Pulsanti + Shadows) ---
st.markdown(f"""
    <style>
...
# BOX
    ...
    if q == "Standard":
        for s, c, d in [("LUNA 🌙", "box-luna", "Neutro"), ("SOLE ☀️", "box-sole", "Vivace"), ("NUVOLA ☁️", "box-nuvola", "Grigio")]:
            ...
            if st.button(f"Scegli {s} ({tg_u})", key=s): aggiungi_al_carrello(f"Box {s}", 19.90)
    else:
...
# CHI SIAMO
    ...
    st.markdown('<div class="card" style="background-color: #FFF1F2; border: 1px solid #fecdd3;"><b style="color:#f43f5e; font-weight:800;">Il nostro obiettivo?</b><br>Offrirti vestiti di qualità, farti risparmiare più di 1000€ l\'anno e lasciare un mondo migliore ai nostri figli.</div>', unsafe_allow_html=True)
...
c = st.columns(7) # 7 Colonne per includere tutto
