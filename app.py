...
# Indicatore carrello rapido
if len(st.session_state.carrello) > 0:
    if st.button(f"🛒 Vai al Carrello ({len(st.session_state.carrello)})"): vai("Carrello"); st.rerun()
...
if st.session_state.pagina == "Home":
    ...
    st.markdown('<p style="text-align:center; padding:0 20px; font-size:13px; color:#475569;">Acquista capi da tenere per sempre. Perfetto per il tuo bimbo o per un regalo!</p>', unsafe_allow_html=True)
    st.selectbox("Filtra per taglia (opzionale):", ["Tutte le taglie", "50-56 cm", "62-68 cm", "74-80 cm", "86-92 cm"], index=0)
    st.markdown('<div class="card">👕 <b>Body Bio "Mamma è la migliore"</b><br><span class="prezzo-rosa">9,90€</span></div>', unsafe_allow_html=True)
    ...
    if not st.session_state.carrello:
        ...
        for i, item in enumerate(st.session_state.carrello):
            st.markdown(f"""<div style="display:flex; justify-content:space-between; padding:10px; border-bottom:1px solid #EEE;">
                <b>{item['nome']}</b> <span>{item['prezzo']:.2f}€</span></div>""", unsafe_allow_html=True)
            totale += item['prezzo']
        st.markdown(f"<h3 style='text-align:right; margin-top:20px;'>Totale: {totale:.2f}€</h3>", unsafe_allow_html=True)
        if st.button("PROCEDI ALL'ORDINE"): 
            st.success("Richiesta ricevuta! Ti contatteremo a breve per confermare il Locker.")
            st.session_state.carrello = []
