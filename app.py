...
def carica_utenti():
    r = requests.get(SHEETDB_API)
    if r.status_code == 200:
        return r.json()
    return []


def registra_utente(email, password, nome_genitore, nome_bambino, taglia):
    payload = {
        "data": [{
            "email": email,
            "password": password,
...
if st.session_state.utente is None:
    ...
    st.markdown("<h2 style='text-align:center;'>🔐 LoopBaby Accesso</h2>", unsafe_allow_html=True)

    scelta = st.radio("Scegli:", ["Login", "Registrazione"])
    ...
    if scelta == "Login":
        ...
        if st.button("ENTRA"):
            ...
            user = login_utente(email, password)
    ...
    # ---------------- REGISTRAZIONE ----------------
        ...
        nome_bambino = st.text_input("Nome Bambino")

        taglia = st.selectbox(
            "Taglia",
            ["50-56 cm", "62-68 cm", "74-80 cm", "86-92 cm"]
        )
        ...
        if st.button("REGISTRATI"):
            ...
            if email and password:
                ...
                if ok:
                    ...
                    st.error("Errore durante registrazione")
