...
def registrazione(email, password):
    try:
        ...
        st.success("📩 Ti abbiamo inviato una mail! Clicca sul link per confermare l'account.")
...
def salva_profilo_su_db(dati_nuovi):
    try:
...
st.markdown(f"""
    ...
    * {{ font-family: 'Lexend', sans-serif !important; }}
    ...
    div.stButton > button {{ background-color: #f43f5e !important; color: white !important; border-radius: 18px !important; width: 100% !important; font-weight: 800 !important; margin: 10px auto !important; display: block !important; white-space: nowrap !important; border: none !important; box-shadow: 0 4px 12px rgba(244, 63, 94, 0.2); }}
...
if st.session_state.user is None:
    ...
    if st.session_state.pagina == "Home":
        st.markdown('<div class="header-custom"><div class="header-text">LOOPBABY</div></div>', unsafe_allow_html=True)
        u_nome = st.session_state.dati.get('nome_genitore', 'Genitore').split()[0]
        ...
        st.markdown(f"""<div style="padding: 0 25px; font-size: 14px; color: #475569; line-height: 1.6;">
                1. <b>Le nostre opzioni:</b> Box Standard o Premium.<br>2. <b>Scegli e ricevi:</b> Nel locker più vicino.<br>3. <b>Controllo 48h:</b> Per problemi <a href="/?nav=Contatti" target="_self" class="link-inline"><b>contattaci</b></a>.<br>4. <b>Dopo 3 mesi:</b> Rendi o ricevi la nuova taglia.</div>""", unsafe_allow_html=True)
        st.markdown('<div class="card"><b>📍 La Regola del 10:</b> Rendi 10 capi per riceverne 10.</div>', unsafe_allow_html=True)
        ...
        if st.button("PAGA ORA"): st.success("Pagamento completato!"); st.session_state.carrello = []
        ...
        if not st.session_state.edit_mode:
            st.markdown(f"""<div class="card" style="text-align:left;">
                ...
                <b>📏 Taglia:</b> {st.session_state.dati.get('taglia', 'Da scegliere')}
            ...
            with st.form("e"):
                ...
                if st.form_submit_button("SALVA"):
                    salva_profilo_su_db({"nome_genitore": n, "nome_bambino": nb, "taglia": tg})
