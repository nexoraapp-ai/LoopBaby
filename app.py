...
# --- 1. CONFIGURAZIONE DATABASE (LINK PULITO) ---
API_URL = "https://sheetdb.io"

def carica_db():
    try:
        # Per LEGGERE usiamo il timestamp (qui è permesso)
...
def aggiungi_utente(nuovo):
    # Per SCRIVERE l'URL deve essere pulito, senza "?t=..."
...
st.markdown(f"""
    ...
    * {{ font-family: 'Lexend', sans-serif !important; }}
    ...
    .header-text {{ color: white; font-size: 32px; font-weight: 800; text-transform: uppercase; text-shadow: 2px 2px 8px rgba(0,0,0,0.3); }}
        ...
        with t1:
            with st.form("l"):
                ...
                if st.form_submit_button("ENTRA"):
                    ...
                    else: st.warning("Nessun utente. Registrati!")
        with t2:
            with st.form("r"):
                ...
                if st.form_submit_button("CREA ACCOUNT"):
                    if er and pr:
                        ...
                        nuovo = {"email": er.strip(), "password": pr.strip(), "nome": "Mamma", "taglia": "---", "inizio": datetime.now().strftime("%Y-%m-%d"), "scadenza": scad, "locker": "Da impostare"}
                        status = aggiungi_utente(nuovo)
                        if status == 201:
                            st.success("✅ REGISTRATA! Ora puoi accedere.")
                            st.balloons()
                        else: st.error(f"Errore tecnico {status}. Controlla i permessi su SheetDB.")
...
# --- APP DOPO LOGIN ---
    ...
    if st.session_state.pagina == "Home":
        ...
        st.markdown(f"""<div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 15px; padding: 0 20px;"><div><div style="font-size:28px; font-weight:800;">Ciao {n}! 👋</div><div style="font-size:14px; color:#334155;">Il tuo armadio infinito è pronto.</div></div><div>{img_h}</div></div>""", unsafe_allow_html=True)
        if st.button("Logout"): 
        ...
        st.markdown("""<div class="info-box"><span class="accent">🚚 Locker InPost</span><br>Ritira 24/7 senza aspettare il corriere.</div>
        <div class="info-box"><span class="accent">🧼 Igiene Professionale</span><br>Capi sanificati ad alte temperature per ogni bambino.</div>""", unsafe_allow_html=True)
