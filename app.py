...
# Questa riga forza l'indirizzo corretto per l'autenticazione
OPZIONI = ClientOptions(auth_url=f"{URL_PROGETTO}/auth/v1")

@st.cache_resource
def get_supabase():
    return create_client(URL_PROGETTO, CHIAVE_ANON, options=OPZIONI)
            ...
            with st.form("l"):
                ...
                if st.form_submit_button("ENTRA"):
                    try:
                        ...
                        vai("Home")
                    except: st.error("Dati errati o mail non confermata.")
        with t2:
            with st.form("r"):
                ...
                if st.form_submit_button("CREA ACCOUNT"):
                    try:
                        # Qui il comando ora punterà correttamente al TUO database
                        supabase.auth.sign_up({"email": er, "password": pr})
                        st.success("📩 Mail inviata! Clicca sul link ricevuto per attivare l'account.")
                    except Exception as err: st.error(f"Errore tecnico: {err}")
else:
    # --- APP DOPO LOGIN ---
    if st.session_state.pagina == "Home":
        st.markdown('<div class="header-custom"><div class="header-text">LOOPBABY</div></div>', unsafe_allow_html=True)
        st.markdown(f"### Ciao! 👋 Benvenuta nel tuo armadio circolare.")
