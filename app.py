...
logo_bg = get_base64("logo.jpg") # Carica l'immagine scelta per il logo

# --- 3. CSS TOTALE (BEIGE + NUOVO HEADER CON SFONDO) ---
st.markdown(f"""
    <style>
...
# HEADER VISIBILE SU OGNI SCHERMATA
st.markdown('<div class="header-banner"><div class="logo-text">LOOPBABY</div></div>', unsafe_allow_html=True)
...
if st.session_state.pagina == "Home":
    ...
    with st.form("form_promo"):
        ...
        st.text_input("Dimensioni (es. 30x30x40 cm)")
        if st.form_submit_button("INVIA E RICHIEDI ETICHETTA"): st.success("Richiesta ricevuta!")
    ...
    st.markdown(f"""<div style="padding: 0 20px; font-size: 13px; color: #475569; line-height: 1.6;"><b>1. Opzioni:</b> Box <b>Standard</b> (usato ottimo) o <b>Premium</b> (nuovi).<br><br><b>2. Scegli e ricevi:</b> Nel locker più vicino a te.<br><br><b>3. Controllo 48h:</b> Controlla i capi, per qualsiasi problema <b>contattaci</b>.<br><br><b>4. Dopo 3 mesi:</b> Scegli se rendere o ricevere la nuova taglia.</div>""", unsafe_allow_html=True)
    ...
    st.markdown("""<div class="card" style="text-align:left; font-size:13px; color:#475569; line-height:1.6;">La Box ha un costo di 19,90€ o 29,90€. Il ritiro è GRATUITO con rinnovo ordine. <b>📍 Regola del 10:</b> Rendi 10 capi per riceverne 10.</div>""", unsafe_allow_html=True)
    ...
    st.markdown("""<div style="text-align:center; padding: 0 20px; color:#475569; font-size:14px; margin-bottom:20px;">I capi acquistati in Vetrina rimarranno nell'armadio del tuo bimbo <b>per sempre</b>.</div>""", unsafe_allow_html=True)
    ...
    if not st.session_state.edit_mode:
        ...
        with st.form("edit"):
            nome = st.text_input("Nome", st.session_state.dati['nome_genitore'])
            locker = st.text_input("Locker", st.session_state.dati['locker'])
            if st.form_submit_button("SALVA"): 
                st.session_state.dati['nome_genitore'] = nome; st.session_state.dati['locker'] = locker
    ...
    st.markdown("""<div class="card" style="background-color:#fff1f2;"><b>Il nostro obiettivo?</b><br>Risparmio, qualità e meno sprechi.</div>""", unsafe_allow_html=True)
