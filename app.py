...
# --- 1. MOTORE DATABASE (VERIFICATO) ---
API_URL = "https://sheetdb.io"

def carica_db():
    try:
        # Forziamo il caricamento senza cache per vedere subito i nuovi iscritti
            ...
            with st.form("l"):
                ...
                if st.form_submit_button("ENTRA"):
                    if not df_globale.empty:
                        # Login robusto (ignora .0 di Excel)
                        df_globale['p_clean'] = df_globale['password'].astype(str).str.split('.').str[0]
                        m = df_globale[(df_globale['email'].str.lower() == e) & (df_globale['p_clean'] == p)]
                        if not m.empty:
                            st.session_state.user = m.iloc[-1].to_dict()
                            vai("Home")
                        else: st.error("Email o Password errati.")
                    else: st.error("Nessun utente trovato. Registrati prima!")
        with t2:
            with st.form("r"):
...
# --- APP DOPO LOGIN ---
    ...
    if st.session_state.pagina == "Home":
        ...
        st.markdown(f"""<div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 15px; padding: 0 20px;">
            <div><div style="font-size:28px; font-weight:800;">Ciao {nome_u}! 👋</div><div style="font-size:14px; color:#334155;">Il tuo armadio infinito è pronto.</div></div>
        ...
        st.markdown("""
        <div class="info-box"><span class="accent">🚚 Locker InPost</span><br>Consegna in 3-5 giorni nel Locker scelto. Ritira 24/7 senza aspettare corrieri.</div>
