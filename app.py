...
# --- 3. IMMAGINI E STILE CSS (LOOK DI IERI) ---
def get_base64(path):
        ...
        with tab1:
            with st.form("l"):
                ...
                if st.form_submit_button("ENTRA"):
                    if not df_db.empty:
                        hp = hash_password(p)
                        # Pulizia password excel (toglie .0 se interpretata come numero)
                        df_db['pass_clean'] = df_db['password'].astype(str).str.replace('.0', '', regex=False).str.strip()
                        match = df_db[(df_db['email'].str.lower() == e) & (df_db['pass_clean'] == hp)]
                        if not match.empty:
        ...
        with tab2:
            with st.form("r"):
                ...
                if st.form_submit_button("CREA ACCOUNT"):
                    if re and rp:
                        nuovo = {"email": re.strip().lower(), "password": hash_password(rp), "nome": rn, "bimbo": rb, "taglia": rt, "inizio": str(date.today()), "scadenza": "", "locker": rl}
                        registra_user(nuovo)
                        st.success("Registrato! Fai l'Accedi.")
                        st.balloons()
...
# --- 5. APP DOPO LOGIN ---
    ...
    if st.session_state.pagina == "Home":
        ...
        <div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 15px; padding: 0 20px;">
            <div>
                <div style="font-size:28px; font-weight:800; color:#1e293b;">Ciao {nome_m}! 👋</div>
        ...
        if st.button("Scopri come"): vai("Info")
