...
def invia_codice_mail(destinatario, codice, nome_mamma):
    ...
    corpo = f"Ciao {nome_mamma}! ✨ Il tuo codice LoopBaby è: {codice}"
...
if "user_data" not in st.session_state: 
    st.session_state.user_data = {"nome": "Mamma", "cognome": "", "email": "", "bimbo": "Bimbo", "sesso": "Neutro", "cellulare": "", "taglia": "0-1m"}
...
if not st.session_state.autenticato:
    ...
    if st.session_state.verificando_mail:
        ...
        cod_in = st.text_input("Codice ricevuto", key="verify_cod")
...
if scelta == "🏠 Home":
    ...
    st.markdown('<div class="card"><h4>L\'Obiettivo di LoopBaby 🌿</h4><p>Risparmio di Tempo, Denaro e Spazio.</p></div>', unsafe_allow_html=True)

# (Aggiungeremo le altre sezioni una per una nei prossimi passaggi)
