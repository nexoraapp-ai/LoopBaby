...
# Logica Welcome: se l'utente non ha mai salvato il nome, mostra Welcome
...
# --- 3. CSS TOTALE (LOOK PROFESSIONALE + SHADOWS) ---
st.markdown(f"""
    <style>
    ...
    * {{ font-family: 'Lexend', sans-serif !important; }}
    ...
    .card {{ border-radius: 25px; padding: 20px; margin: 10px 20px; border: 1px solid #EAE2D6; text-align: center; background-color: #FFFFFF; box-shadow: 0 8px 25px rgba(0,0,0,0.04); }}
...
if st.session_state.pagina == "Welcome":
    ...
    st.markdown("<p style='text-align:center; padding:0 30px;'>L'armadio circolare che cresce con il tuo bambino. Iniziamo subito a risparmiare insieme!</p>", unsafe_allow_html=True)
    with st.form("w_form"):
        ...
        if st.form_submit_button("ENTRA NELL'APP"):
            if nome:
                ...
                vai("Home"); st.rerun()
            else: st.error("Per favore, inserisci il tuo nome per iniziare.")
...
# 2. PROMO DETTAGLIO
    ...
    st.markdown('<div class="card" style="text-align:left; font-size:14px;"><b>Preparare il pacco è semplicissimo:</b> mandaci almeno <b>10 capi</b> in buono stato e ti regaliamo la tua prima Box!</div>', unsafe_allow_html=True)
    with st.form("promo_f"):
        ...
        if st.form_submit_button("RICHIEDI ETICHETTA PREPAGATA"): 
...
# 5. VETRINA (LIBERA)
    ...
    st.markdown('<p style="text-align:center; padding:0 20px; font-size:13px;">Acquista capi da tenere per sempre. Perfetto per un regalo!</p>', unsafe_allow_html=True)
    ...
    if st.button("Aggiungi al carrello 🎁"): aggiungi_al_carrello("Body Bio Mamma", 9.90)

# 6. CARRELLO E PAGAMENTO
elif st.session_state.pagina == "Carrello":
    st.markdown('<h2 style="text-align:center;">Il tuo Carrello 🛒</h2>', unsafe_allow_html=True)
    if not st.session_state.carrello:
        st.warning("Il carrello è vuoto. Torna a scegliere le tue Box!")
...
# 8. CHI SIAMO E CONTATTI
    ...
    st.markdown('<div class="card" style="background-color: #FFF1F2;"><b style="color:#f43f5e;">Il nostro obiettivo?</b><br>Offrirti qualità e farti risparmiare più di 1000€ l’anno.</div>', unsafe_allow_html=True)
