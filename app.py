...
# --- 1. CONNESSIONE SUPABASE (CHIAVE AGGIUNTA) ---
SUPABASE_URL = "https://supabase.co"
# Ho inserito la tua chiave pubblica qui sotto
...
if st.session_state.user is None:
    if st.session_state.pagina == "Welcome":
        ...
        with t2:
            with st.form("r"):
                er, pr = st.text_input("La tua Email"), st.text_input("Password (min 6 car.)", type="password")
...
# APP DOPO IL LOGIN
    ...
    if st.session_state.pagina == "Home":
        st.markdown('<div class="header-custom"><div class="header-text">LOOPBABY</div></div>', unsafe_allow_html=True)
        u_nome = st.session_state.dati.get('nome_genitore', 'Genitore').split()[0] if st.session_state.dati.get('nome_genitore') else "Mamma"
        ...
        st.markdown(f"""<div style="padding: 0 25px; font-size: 14px; color: #475569; line-height: 1.6;">
                1. <b>Le opzioni:</b> Box Standard o Premium.<br>2. <b>Scegli e ricevi:</b> Nel locker più vicino.<br>3. <b>Problemi?</b> Per aiuto <a href="/?nav=Contatti" target="_self" class="link-inline"><b>contattaci</b></a>.</div>""", unsafe_allow_html=True)
        ...
        if st.button("PROCEDI AL PAGAMENTO (STRIPE)"): st.info("Reindirizzamento sicuro...")
        ...
        if not st.session_state.edit_mode:
            ...
            with st.form("edit"):
                ...
                nas = st.date_input("Data Nascita")
                ...
                if st.form_submit_button("SALVA SUL DATABASE"):
                    salva_profilo_db({"nome_genitore": n, "nome_bambino": nb, "nascita": str(nas), "telefono": tel, "taglia": tg, "locker": lock})
