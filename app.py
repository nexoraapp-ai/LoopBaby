...
st.markdown(f"""
    ...
    * {{ font-family: 'Lexend', sans-serif !important; }}
    ...
    .header-custom {{
        ...
        margin-bottom: 35px; border-radius: 0 0 30px 30px; position: relative;
    }}
    ...
    .cart-icon-header {{
        ...
        padding: 8px 12px; border-radius: 50%; font-size: 18px; cursor: pointer; box-shadow: 0 2px 5px rgba(0,0,0,0.2);
    }}
...
# Header con pulsante carrello invisibile (cliccabile tramite colonna Streamlit per semplicità)
st.markdown(f'<div class="header-custom"><div class="header-text">LOOPBABY</div></div>', unsafe_allow_html=True)

# Mostra contatore carrello se non vuoto
if len(st.session_state.carrello) > 0:
    if st.button(f"🛒 Carrello ({len(st.session_state.carrello)} articoli)"): vai("Carrello"); st.rerun()

# --- 4. PAGINE ---

if st.session_state.pagina == "Home":
    img_html = f'<img src="data:image/jpeg;base64,{img_data}" class="baby-photo">' if img_data else ""
    u_nome = st.session_state.dati['nome_genitore'].split() if st.session_state.dati['nome_genitore'] else ""
    ...
    st.selectbox("Taglia:", ["Tutte", "50-56 cm", "62-68 cm", "74-80 cm", "86-92 cm"])
    ...
    if not st.session_state.carrello:
        ...
        for i, item in enumerate(st.session_state.carrello):
            cols = st.columns([3, 1])
            cols[0].write(f"**{item['nome']}**")
            cols[1].write(f"{item['prezzo']}€")
            totale += item['prezzo']
        st.markdown(f"<hr><h3 style='text-align:right;'>Totale: {totale:.2f}€</h3>", unsafe_allow_html=True)
        if st.button("SVUOTA CARRELLO"): st.session_state.carrello = []; st.rerun()
        if st.button("PROCEDI ALL'ORDINE"): st.success("Ordine inviato! Ti contatteremo su WhatsApp."); st.session_state.carrello = []
    ...
    if not st.session_state.edit_mode:
        ...
        with st.form("edit_f"):
            ...
            if st.form_submit_button("SALVA"):
                st.session_state.dati = {"nome_genitore": n, "email": m, "taglia": tg, "locker": lock, "telefono": "", "nome_bambino": "", "nascita": date(2024,1,1)}
