...
with st.sidebar:
    ...
    st.button("🔥 Promo", on_click=lambda: go("Promoimport streamlit as st
import os
import json
import base64
import requests
...
# BLOCCO ACCESSO
# =========================
if not st.session_state.logged and st.session_state.page not in ["Login"]:
...
if st.session_state.page == "Home":
    ...
    with col1:
        st.markdown("""
LoopBaby è un sistema circolare per bambini.
...
""")
    ...
    with col2:
        if baby:
            st.image("bimbo.jpg", width=120, caption=d.get("bimbo", "Bimbo"))
...
if st.session_state.page == "Promo":
    ...
    st.markdown("Dona 10 capi → Box gratis")
...
if st.session_state.page == "Box":
    ...
    if tipo == "Standard":
        if st.button("Aggiungi Box"):
            st.session_state.cart.append({"name": "Box Standard", "price": 14.90})

    else:
        if st.button("Aggiungi Premium"):
            st.session_state.cart.append({"name": "Box Premium", "price": 24.90})
...
if st.session_state.page == "Info":
    st.title("ℹ️ Info LoopBaby")
    st.markdown("Sistema circolare per vestiti bambini.")
...
if st.session_state.page == "Profilo":
    ...
    if st.button("Salva"):
        st.success("Salvato (locale sessione)")
    st.button("👤 Profilo", on_click=lambda: go("Profilo"))
