    ...
    if scelta == "Registrati":
        ...
        if st.button("CREA ACCOUNT"):
            if email and password:
                if registra_utente(email, password, rn, rb, rt, rl) == 201:
...
if st.session_state.pagina == "Home":
    ...
    u_n = st.session_state.user_data.get('nome_genitore', 'Mamma').split()[0]
    st.markdown(f"""<div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 15px; padding: 0 20px; align-items: start;">
        <div>
            <div style="font-size:28px; font-weight:800; color:#1e293b;">Ciao {u_n}! 👋</div>
    ...
    st.markdown("""<div style="padding: 0 25px; font-size: 14px; color: #475569; line-height: 1.6;">
            ...
            <b>3. Utilizzo:</b> Massimo 3 mesi.</div>""", unsafe_allow_html=True)
    st.markdown('<h2 style="text-align:center;">Il Patto del 10 📍</h2>', unsafe_allow_html=True)
    st.markdown('<div class="card" style="text-align:left; font-size:13px;">♻️ Rendi 10 capi per riceverne 10 nuovi.</div>', unsafe_allow_html=True)

elif st.session_state.pagina == "Box":
    tg = st.session_state.user_data.get('taglia', '50-56 cm')
    st.markdown(f'<div style="text-align:center;">📍 TAGLIA ATTIVA: {tg}</div>', unsafe_allow_html=True)
    ...
    if q == "Standard":
        ...
        st.markdown('<div class="card box-premium"><h3>PREMIUM 💎</h3><div style="font-size:28px; font-weight:900;">29,90€</div></div>', unsafe_allow_html=True)
