...
if not st.session_state.loggato:
    ...
    scelta = st.radio("Accesso", ["Accedi", "Registrati"], horizontal=True, key="auth_mode")

    email = st.text_input("Email", key="auth_email")
    password = st.text_input("Password", type="password", key="auth_pass")

    if scelta == "Registrati":
        if st.button("CREA ACCOUNT", key="btn_register"):
    ...
    if scelta == "Accedi":
        if st.button("ENTRA", key="btn_login"):
            ...
            if user:
                ...
                # FIX SICURO CAMPI MANCANTI
                st.session_state.dati.setdefault("telefono","")
                st.session_state.dati.setdefault("nascita","")
                st.session_state.dati.setdefault("nome_genitore","")
                st.session_state.dati.setdefault("nome_bambino","")
                st.session_state.dati.setdefault("taglia","50-56 cm")
                st.session_state.dati.setdefault("locker","")
