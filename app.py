...
# --- 1. MOTORE DATABASE (CON CACHE BUSTER) ---
API_URL = "https://sheetdb.io"

def carica_db():
    try:
        # Forziamo il caricamento dati freschi per vedere subito le nuove mamme
            ...
            with st.form("l"):
                ...
                if st.form_submit_button("ENTRA"):
                    if not df_globale.empty:
                        # Pulizia dati Excel (.0 delle password)
...
# --- APP DOPO LOGIN ---
    ...
    if st.session_state.pagina == "Home":
        ...
        <div class="info-box"><span class="accent">🔄 Cambio Taglia</span><br>Quando i vestiti stringono, ordina la nuova taglia. Scambiamo il vecchio col nuovo.</div>
        ...
        if not st.session_state.edit_mode:
            st.markdown(f"""<div class="card" style="text-align:left;">
                <b>👤 Genitore:</b> {st.session_state.user.get('nome')}<br>
        ...
        st.markdown('<div class="card" style="text-align:left; line-height:1.6;"><b>Basta vestiti ammassati in soffitta.</b><br><br>Siamo genitori che hanno creato LoopBaby per far viaggiare la qualità di famiglia in famiglia, seguendo la crescita dei piccoli senza sprechi.🌿</div>', unsafe_allow_html=True)
