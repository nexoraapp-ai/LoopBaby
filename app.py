...
def carica_db():
    try:
        # Forziamo il caricamento dati freschi saltando la cache di SheetDB
        timestamp = datetime.now().timestamp()
        res = requests.get(f"{API_URL}?t={timestamp}", timeout=10)
        if res.status_code == 200:
            df = pd.DataFrame(res.json())
            if not df.empty:
                # PULIZIA TOTALE: toglie spazi e rende tutto minuscolo per il codice
...
st.markdown(f"""
    ...
    * {{ font-family: 'Lexend', sans-serif !important; }}
    .header-custom {{ background-image: linear-gradient(rgba(0,0,0,0.1), rgba(0,0,0,0.1)), url("data:image/png;base64,{logo_bg}"); background-size: cover; height: 130px; display: flex; align-items: center; justify-content: center; margin-bottom: 35px; border-radius: 0 0 30px 30px; }}
            ...
            with st.form("login_f"):
                ...
                if st.form_submit_button("ENTRA"):
                    if not df_globale.empty and 'email' in df_globale.columns:
                        # LOGIN ROBUSTO: pulisce spazi e ignora maiuscole
                        user_match = df_globale[
                            (df_globale['email'].astype(str).str.strip().str.lower() == e.strip().lower()) & 
                            (df_globale['password'].astype(str).str.strip() == str(p).strip())
                        ]
        ...
        with t2:
            with st.form("reg_f"):
                ...
                if st.form_submit_button("CREA ACCOUNT"):
                    if not df_globale.empty and er.strip().lower() in df_globale['email'].astype(str).str.lower().values:
                        st.error("Sei già registrata!")
                    else:
                        scad = (datetime.now() + timedelta(days=90)).strftime("%Y-%m-%d")
                        nuovo = {"email": er.strip(), "password": pr.strip(), "nome genitore": "Mamma", "taglia": "---", "data inizio": datetime.now().strftime("%Y-%m-%d"), "scadenza": scad, "locker": "Da impostare"}
...
# --- APP DOPO LOGIN ---
    ...
    if st.session_state.pagina == "Home":
        ...
        st.markdown(f"""<div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 15px; padding: 0 20px;">
            <div><div style="font-size:28px; font-weight:800; color:#1e293b;">Ciao {nome}! 👋</div><div style="font-size:14px; color:#334155;">Il tuo armadio circolare è pronto.</div></div>
        ...
        if st.button("Scopri come funziona"): vai("Info")
    ...
    # --- INFO LOGISTICA ---
        ...
        <div class="info-box">
            ...
            Quando i vestiti iniziano a stare stretti, ordina la taglia successiva dall'app. Ti spediremo la nuova Box e potrai consegnare quella vecchia nello stesso momento.
        </div>
        <div class="info-box">
            <span class="accent">🧼 Igiene e Qualità</span><br>
            Ogni capo rientrato viene <b>sanificato professionalmente</b> ad alte temperature con prodotti ipoallergenici certificati. Riceverai solo vestiti pronti per essere indossati.
        </div>
        """, unsafe_allow_html=True)
    ...
    # --- CHI SIAMO ---
        ...
        <div class="card" style="text-align:left; line-height:1.6; font-size:15px;">
            ...
            <b>LoopBaby</b> nasce per questo: creare un "giro infinito" di abbigliamento di alta qualità. I capi viaggiano di famiglia in famiglia, seguendo la crescita dei bimbi senza pesare sul portafoglio dei genitori e sull'ambiente.🌿
        </div>
        """, unsafe_allow_html=True)

    # --- PROFILO (LOCKER E TASTO SALVA) ---
        ...
        with st.form("p"):
            ...
            loc = st.text_input("Locker InPost preferito (Città/Indirizzo)", st.session_state.user.get('locker', ''))
