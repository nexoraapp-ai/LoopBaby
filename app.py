...
def carica_db():
    try:
        ...
        if res.status_code == 200:
            ...
            if not df.empty:
                # Normalizziamo i nomi delle colonne per farli combaciare col codice
        ...
        with tab2:
            with st.form("r"):
                ...
                if st.form_submit_button("Crea account"):
                    nuovo = {"email": re, "password": hash_password(rp), "nome_genitore": rn, "nome_bambino": rb, "taglia": rt, "data_inizio": str(date.today()), "scadenza": "", "locker": rl}
                    registra_user(nuovo)
...
# --- 5. APP DOPO LOGIN ---
    ...
    if st.session_state.pagina == "Home":
        ...
        st.markdown('<div class="card" style="text-align:left; font-size:13px;"><b>♻️ Equilibrio:</b> Rendi 10 capi per riceverne 10 nuovi.<br><br><b>👖 Sostituzione:</b> Se rovini un capo, sostituiscilo con uno tuo o paga 5€.</div>', unsafe_allow_html=True)
        ...
        if not st.session_state.carrello: st.write("Il carrello è vuoto.")
            ...
            if st.button("PAGA ORA"): st.success("Reindirizzamento...")
        ...
        </div><div class="obiettivo-pink"><b>Il nostro obiettivo?</b><br>Risparmi più di 1000€ l'anno e lasci un mondo migliore.</div>""", unsafe_allow_html=True)
